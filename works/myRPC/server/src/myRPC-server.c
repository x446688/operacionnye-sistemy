  ...
  if (sig == SIGINT)
    {
      ...
      stop = 1;
      _LOG ("Received SIGINT, stopping daemon...", LOG_LVL_DEBUG);
      /* Reset signal handling to default behavior */
      signal (SIGINT, SIG_DFL);
    }
  else if (sig == SIGHUP)
    {
      _LOG ("Received SIGHUP, reloading daemon config file ...",
            LOG_LVL_DEBUG);
      get_parsed_config_options (server_conf_file);
    }
  else if (sig == SIGCHLD)
    {
      _LOG ("Received SIGCHLD, received SIGCHLD signal", LOG_LVL_DEBUG);
    }
}
        ...
        if (user_allowed (username) == 0)
          {
            ...
            if (cmd_return_code == 0)
              {
                _LOG ("Command returned success", LOG_LVL_INFO);
          ...
          {
            snprintf (result, BSIZE, "User '%s' is not allowed.",
                      username);
            json_object_object_add (response_root, "code",
        ...
        if (use_stream)
          {
            if (send (connfd, response, strlen (response), 0) == -1 )
              {
                printf("%s\n",strerror(errno));
                _LOG ("Error sending response to client (stream)",LOG_LVL_ERROR);
              }
              close (connfd); 
          }
        else 
          {
            if (sendto (sockfd, response, strlen (response), 0,
                  ...
                  {
                    printf("%s\n",strerror(errno));
                    _LOG ("Error sending response to client (dgram)",LOG_LVL_ERROR);
                  }
          }
        }
  close (sockfd);
