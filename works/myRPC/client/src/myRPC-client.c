...
int main(int argc, char *argv[]) {
    ...
    mysyslog("Connecting to the server...",LOG_LVL_INFO, 1, 1, "/var/log/myrpc.log");
    int sockfd;
    if (use_stream) {
    ...
    if (sockfd < 0) {
        mysyslog("Socket creation failed", LOG_LVL_ERROR, 1, 1, "/var/log/myrpc.log");
    ...
    if (use_stream) {
        if (connect(sockfd, (struct sockaddr*)&servaddr,sizeof(servaddr)) < 0) {
            mysyslog("Connection failed", LOG_LVL_ERROR, 1, 1, "/var/log/myrpc.log");
            perror("Connection failed");
            close(sockfd);
            return 1;
        }
        mysyslog("Successfully connected to server", LOG_LVL_INFO, 1, 1, "/var/log/myrpc.log");
        send(sockfd, request, strlen(request), 0);
        char response[BSIZE];
        socklen_t len = sizeof(servaddr);
        int n = recvfrom(sockfd, response, BSIZE, 0, (struct sockaddr*)&servaddr, &len);
        response[n] = '\0';
        printf("Server response: %s\n", response);
        mysyslog("Received server response", LOG_LVL_INFO, 1, 1, "/var/log/myrpc.log");
    }
    close(sockfd);
    return 0;
}