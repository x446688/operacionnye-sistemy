 ...
 *
 */
/* This program (libmysyslog-daemon)is based on the daemon example
provided by Jiri Hnidek on github. It is a systemd service that 
tests capabilites of libmysyslog by randomly sending messages to a 
specified logfile.*/
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
const char *passed_filepath = NULL;
const char *msg = NULL;
static int level = 0;
static int driver = 0;
static int format = 0;
 ...
 */
int
test_conf_file (char *_conf_file_name)
{
  FILE *conf_file = NULL;
...
main (int argc, char *argv[])
{
  ...
  while ((value =
          getopt_long (argc, argv, "c:l:t:p:dh", long_options,
                       &option_index)) != -1)
    {
      switch (value)
        {
  ...
  while (running == 1)
    {
      ...
      mysyslog ("sample text", lvl_val, driver_val, format_val,
                passed_filepath);
      sleep (delay);
    }
